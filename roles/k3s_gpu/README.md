...
Single-node k3s install. Opt-in via `k3s_gpu_install: true` at the playbook
...
|---|---|---|
| `k3s_gpu_install` | `false` | Gate the whole role. |
| `k3s_gpu_operator_install` | `false` | Apply the NVIDIA device plugin DaemonSet. |
| `k3s_gpu_version` | `v1.30.2+k3s1` | Pinned for reproducibility. |
| `k3s_gpu_device_plugin_version` | `v0.15.0` | NVIDIA k8s device plugin manifest tag. |

Vault: `vault_k3s_token` (required when `k3s_gpu_install: true`).
